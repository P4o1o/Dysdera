...
class SelectionPolicy:
    ...
    @staticmethod
    def must_contain(word: str) -> Callable[[WebTarget], bool]:
        def final_policy(target: WebTarget) -> bool:
            return word.lower() in target.url().lower()

        return final_policy

    @staticmethod
    def same_domain(of_url: str) -> Callable[[WebTarget], bool]:
    ...
    @staticmethod
    def not_true(policy: Callable[[WebTarget], bool]) -> Callable[[WebTarget], bool]:
        def final_policy(target: WebTarget) -> bool:
            return not policy(target)

        return final_policy

    @staticmethod
    def all_true(*policy: Callable[[WebTarget], bool]) -> Callable[[WebTarget], bool]:
        def final_policy(target: WebTarget) -> bool:
            return all(func(target) for func in policy)

        return final_policy

    @staticmethod
    def at_least_one_true(*policy: Callable[[WebTarget], bool]) -> Callable[[WebTarget], bool]:
        def final_policy(target: WebTarget) -> bool:
            return any(func(target) for func in policy)
...
class SelectionPolicyWithHeaders:
    @staticmethod
    def modify_only_before(date: datetime, if_date_absent=False) -> Callable[[WebTarget], bool]:  # Python 3.9+
    ...
    @staticmethod
    def modify_only_after(date: datetime, if_date_absent=False) -> Callable[[WebTarget], bool]:
        date_stamp = absolute_timestamp(date)
    ...
    @staticmethod
    def modify_between(start: datetime, end: datetime, if_date_absent=False) -> Callable[[WebTarget], bool]:
    ...
    @staticmethod
    def is_html() -> Callable[[WebTarget], bool]:
        def final_policy(target: WebTarget) -> bool:
